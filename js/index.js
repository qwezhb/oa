...
//				 $('.imgtop').css('background-image',"url(img/images/bz"+json.result+".jpg)");
                $('.imgtoplist li').eq(json.result).fadeIn();
                 $('.imgtoplist li').eq(json.result).addClass('liCur');
                 $('.imgtopul li').eq(json.result).addClass('toucur');
			},
...
//图片选择
requirejs(['jquery'],function($){
	var lisNode=$('.imgtoplist li');
	var licNode=$('.imgtopul li');
...
//		}
//	}
         var oldpos=$('.imgtoplist .liCur').index();
     	var nowpos;
     	nowpos=oldpos==lisNode.length-1?0:oldpos+1;
  changeimg(oldpos,nowpos);
	})
...
//      $('.imgtop').css('background-image',sd);//赋值新图
     	...
     	function changeimg(oldpos,nowpos)
     	{
     		lisNode.eq(nowpos).addClass('liCur');
     	    lisNode.eq(oldpos).removeClass('liCur');
     	    licNode.eq(nowpos).addClass('toucur');
     	    licNode.eq(oldpos).removeClass('toucur');
     	    lisNode.eq(nowpos).fadeIn();
     	    lisNode.eq(oldpos).fadeOut();
...
requirejs(['jquery'],function($){
		$('.refound-se').change(function(){
			var text=$('.refound-type select').val();
			$('.refound-type span').html(text);
		});
		$('.refound-sen').change(function(){
			var name=$('.refound-name select').val();
			$('.refound-name span').html(name);
		})
})
